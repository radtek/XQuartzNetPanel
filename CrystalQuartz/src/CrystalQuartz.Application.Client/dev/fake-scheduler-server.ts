...
export class FakeSchedulerServer {
    ...
    constructor(options: IFakeSchedulerOptions) {
        ...
        this._commandHandlers = {
            ...
            'get_job_details': (args) => ({
                _ok: true,
                jd: {
                    ced: true,                        // ConcurrentExecutionDisallowed 不允许同时执行
                    ds: '',                           // Description 描述
                    pjd: false,                       // PersistJobDataAfterExecution 执行后保留job数据
                    d: false,                         // Durable 持久
                    t: 'SampleJob|Sample|InBrowser',  // JobType job类型
                    rr: false                         // RequestsRecovery 可恢复性
                },
                jdm: {
                    '_': 'object',
                    v: {
            ...
            'add_trigger': (args) => {
                ...
                if (triggerType !== 'Simple') {
                    return {
                        _err: '只支持“简单”触发器类型 by in-browser fake scheduler implementation'
                    };
                }
    ...
    handleRequest(data) {
        ...
        return { _err: 'Fake scheduler server 不支持命令' + data.command };
    }
